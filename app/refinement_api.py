...
from app.api import _decorate_search_items, err, ok
...
def parse_search_terms(value: str, *, limit: int = 6) -> list[str]:
    """Split a human query into stable, de-duplicated search terms."""
    terms: list[str] = []
    seen: set[str] = set()
...
def _source_queries(query: str, terms: list[str], mode: str) -> list[str]:
    if mode == "raw":
        return [query.strip()]
...
def _purge_search_cache(queries: list[str], order: str, page: int) -> None:
    ...
    with lock:
        for key in list(cache):
            if (
                ...
                and len(key) >= 3
                and str(key[0]).casefold() in wanted
                and str(key[1]) == order
                and int(key[2]) == page
            ):
                cache.pop(key, None)
...
def _item_matches(item: dict[str, Any], terms: list[str], mode: str) -> bool:
    if mode == "raw" or not terms:
        return True
    haystack = " ".join(
        str(item.get(field) or "") for field in ("title", "author", "bvid")
    ).casefold()
    matches = [term.casefold() in haystack for term in terms]
    return all(matches) if mode == "all" else any(matches)


def search_videos_by_mode(
    query: str,
    *,
    mode: str,
    order: str,
    ...
    if canonical_mode != "raw" and not terms:
        raise SearchError("没有可用于词搜索的关键词")

    queries = _source_queries(query, terms, canonical_mode)
    if fresh:
        _purge_search_cache(queries, order, page)
    ...
    for source_query in queries:
        ...
        except Exception as exc:  # noqa: BLE001 - retain partial term results
            first_error = first_error or exc
    if not sources:
        if first_error:
            raise first_error
        raise SearchError("没有取得搜索结果")

    merged: dict[str, dict[str, Any]] = {}
    for source in sources:
        for item in source.get("items") or []:
            bvid = str(item.get("bvid") or "").strip()
            if bvid and bvid not in merged:
                merged[bvid] = dict(item)
    items = [
        item
        for item in merged.values()
        if _item_matches(item, terms, canonical_mode)
    ]
    pages = max((int(source.get("pages") or 0) for source in sources), default=0)
    totals = [int(source.get("total") or 0) for source in sources]
    total = max(totals, default=len(items))
    candidate_total = sum(totals)
    normalized = " ".join(terms) if canonical_mode != "raw" else query
    cached = bool(sources) and all(bool(source.get("cached")) for source in sources)
    return {
        "keyword": query,
