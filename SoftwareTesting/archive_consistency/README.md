...
每个归档区必须有 UTF-8、LF 的 `README.md` 三列表格。每份归档 Markdown 必须由本区索引恰好登记一次；索引不得重复、越界或链接不存在的正文。当前承接真源只能是一个项目内活动 Markdown，或精确写“无，仅保留历史证据”。归档正文中的普通相对链接和标题锚点必须有效。

T-ARCHIVE 不判断历史正文事实是否仍然正确，也不把历史结果提升为当前状态。活动导航、活动方案状态和 Registry 结构由 T-DOC 承接。
