...
- 自动入口只在当前 Git common directory 存在有效 `bili-workspace/automatic-live-test.json` 时使用其中严格绑定的凭据源与测试根；显式入口仍可由调用者逐次提供。自动绑定的凭据源必须有有效 `.bili-workspace-data-root.json` 身份标记；所有模式都要求有效 `.bili-workspace-live-test.json` 固定场景和普通 `config/bbdown/BBDown.data`。运行器不能创建授权或场景、扫描其他目录或读取来源数据库、配置与媒体。
- 测试根必须是仓库外、凭据源外且不位于 `%LOCALAPPDATA%` 的绝对路径。自动入口使用仓库授权中的 `test_root`，显式入口使用 `BILI_TEST_ROOT`；每次只创建一个带通用所有权标记和 `results/summary.json` 的新 run，产品、工具、浏览器 profile、下载和日志全部位于该 run。
- 真实链固定 15 分钟、run 增长 2 GiB、开始下载前 5 GiB 可用空间和 8 个 BV 上限；只停止本次持有句柄的进程。成功停止后删除 run 内所有 `BBDown.data`，非通过 run 整体保留。
- 原始公开响应可以保存在 run，但不得记录请求头、Cookie、二维码载荷、原始 `/nav` 账号响应、WBI 签名查询值、响应头、HAR 或完整网络流量；这些产物和固定场景永不进入 Git，仓库本地授权位于 Git common directory 且不随提交或克隆传播。
